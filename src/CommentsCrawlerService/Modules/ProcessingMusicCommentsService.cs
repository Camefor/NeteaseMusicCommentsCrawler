...
namespace CommentsCrawlerService.Modules
{
    ...
    public class ProcessingMusicCommentsService
    {
        ...
        public void MatchCommntsData(SongsSimpleItem asongs, List<CommentsItem> acommentList)
        {
            var targetList = acommentList.Where(_ => _.user.userId.ToStr() == GlobalStateData.uid)?.ToList();
            if (targetList == null)
            {
                return;
            }
